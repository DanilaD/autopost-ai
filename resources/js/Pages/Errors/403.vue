...
<template>
    ...
    >
        ...
        >
            <!-- Error Code -->
            <div class="text-8xl font-bold text-md-warning">
                {{ status }}
            </div>
            ...
            <div class="space-y-4">
                ...
                <Link
                    :href="route('login')"
                    class="w-full py-3 px-4 text-sm font-medium text-md-on-surface bg-md-surface-container-high hover:bg-md-surface-container focus:outline-none focus:ring-2 focus:ring-md-outline focus:ring-offset-2 transition-all duration-medium2 rounded-md inline-block"
                >
                    {{ t('auth.login') }}
                </Link>
            </div>
