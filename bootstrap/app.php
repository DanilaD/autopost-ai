...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        ...
        // Add session middleware to API routes for Inertia compatibility
    ...
    ->withExceptions(function (Exceptions $exceptions): void {
        ...
        // Handle Authentication errors - redirect to login with message
        $exceptions->render(function (\Illuminate\Auth\AuthenticationException $e, $request) {
            if ($request->expectsJson()) {
                return response()->json(['message' => 'Authentication required'], 401);
            }

            // For AI routes, redirect to login with a specific message
            if (str_starts_with($request->path(), 'ai')) {
                return redirect()->route('login')->with('status', __('auth.login_required'));
            }
